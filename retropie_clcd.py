...
mylcd.lcd_display_string("Hakjong Wolrd", 2, 0)
sleep(5) # 5 sec delay
mylcd.lcd_clear()
...
mylcd.lcd_display_string("blog.naver.com/", 1, 0)
mylcd.lcd_display_string("i032co2p", 2, 0)
sleep(5) # 5 sec delay
mylcd.lcd_clear()
...
#      except FileNotFoundError:
         ...
         mylcd.lcd_display_string( "%s" %(rom), 2, 0 )
         sleep(3)
         f.close() 
         ...
         while True :
            if wait < 10 and system != flag:
               ...
               mylcd.lcd_display_string( "%s" %(system), 1, 0 )
               mylcd.lcd_display_string( "%s" %(message), 2, 0 )
               sleep(speed)
               wait = wait + 0.1
            else :
               break
